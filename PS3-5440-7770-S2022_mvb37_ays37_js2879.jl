...
# ╔═╡ decd2900-1b33-401d-a57f-64797242b2db
#Worked on by Jonathan, Malini and Alison
# Malini and Alison worked on developing a systematic method to calculate the flux bounds
# Jonathan worked on the code to implement the method
...
md"""
Add additional column to stoichiometric array. Make mu be less than equal to UR+1 so set it to exactly mu. Find mu by using doubling time. 
"""
...
# ╔═╡ 8afdc069-86c1-4341-ac80-203d97714c8c
#Simple case: rate = s/(km+s), S >> km, rate goes to 1. If S << Km, rate goes to S/km
...
begin
 	#NOTE USING ALL ONLY THE HOMOSAPIEN DATA FOR THE REACTANTS (Very little data so any value that does not have an S/K value that is a reactant and homosapien is just equal to 1)
	...
	#Using only the homo sapien data
	EInitial = 0.01 # units: μmol/gDW
	u2 = ones(6)
	Enew = EInitial.*u2
	
	# setup flux bounds array for the new reactions using only the homo sapien data for the reactants
	flux_bounds_array1 = zeros(21,2)
	flux_bounds_array1[:,2] .= 100.0 # default value is 100 for flux units: μmol/gDW-s
	flux_bounds_array1[1,2] = 203.0*Enew[1] *(((1^3)/ ((2)-1)))
	flux_bounds_array1[2,2] = 34.5*Enew[2] *(((1^1)) / ((2^1)-1))
	flux_bounds_array1[3,2] = 249.0*Enew[3]* ( (((1*0.165))) / ((2*1.165)-1))
	flux_bounds_array1[4,2] = 88.1*Enew[4]* ( ((1^2)) / ((2) -1))
	flux_bounds_array1[5,2] = 13.7*Enew[5]* (  (73.1*1)/((74.1*2) -1) )
	flux_bounds_array1[6,2] = 13.7*Enew[6]* (  (((1^4)) / ((2)-1)) )
	flux_bounds_array1
	# delete powers --> no geometric mean
...
begin
		important_df = df[:, ["EC Number", "[Met]/Km"]]
		mna
		species_rxn_df = DataFrame(Metabolite = mna)
		species_rxn_df.:"6.3.4.5" = S[:,1]
		species_rxn_df.:"4.3.2.1" = S[:,2]
		species_rxn_df.:"3.5.3.1" = S[:,3]
		species_rxn_df.:"2.1.3.3" = S[:,4]
		species_rxn_df.:"1.14.13.39F" = S[:,5]
		species_rxn_df.:"1.14.13.39R" = S[:,6]

		missing_value = 1
	
		#dataframe representation of S
		species_rxn_df
	
		col_names = names(species_rxn_df)

		flux_bounds_array2 = zeros(21,2)
		flux_bounds_array2[:,2] .= 100.0 # default value is 100 for flux units: μmol/gDW-s
		flux_bounds_array2[1,2] = 203.0*Enew[1]
		flux_bounds_array2[2,2] = 34.5*Enew[2]
		flux_bounds_array2[3,2] = 249.0*Enew[3]
		flux_bounds_array2[4,2] = 88.1*Enew[4]
		flux_bounds_array2[5,2] = 13.7*Enew[5]
		flux_bounds_array2[6,2] = 13.7*Enew[6]
		...
		for rxn_num in 2:4
			...
			lowercase_strings(isolate_df) #converts metabolite names to lowercase
		
			numerator_acc = 1
			denominator_acc = 1
			for species in species_per_rxn[:, :Metabolite]
				value = missing_value
				for row in 1:size(isolate_df)[1]
				...
				numerator_acc *= value
				denominator_acc *= (1 + value)
				# calculate upper bound flux now with value
			end
